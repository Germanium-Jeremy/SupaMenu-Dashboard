...
const SideMenu = () => {
  ...
  return (
     <div className={`bg-black text-white fixed top-0 left-0 bottom-0 w-[18%] overscroll-y-auto`}>
          <div className='h-full w-full flex flex-col px-[1rem] py-[2rem] overscroll-y-auto'>
               <h2 className={`text-2xl text-center font-bold text-white`}>Supa<span className={`text-amber-500`}>Menu</span></h2>
               <div className={`py-[2rem] font-light mb-[70%]`}>
                    {menuLinks.map((menuLink) => {
                         const isActive = location.pathname.startsWith(menuLink.link);
                         return (
                              <Link key={menuLink.id} to={menuLink.link}
                                   className={`flex gap-5 items-center text-sm px-2 py-[.5rem] ${isActive ? "border-l-4 font-normal bg-gray-950" : "hover:bg-gray-900"}`}
                              >
               ...
               <Link to={`#`}
                    className={`flex gap-3 items-center text-sm px-1 py-[.5rem] font-normal bg-gray-950 hover:bg-gray-900"}`}
               >
                    <span><FaCog /></span>
                    <span>Settings</span>
               </Link>
